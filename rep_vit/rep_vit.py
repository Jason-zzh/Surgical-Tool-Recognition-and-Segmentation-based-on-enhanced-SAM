...
'''try:
    # 尝试绝对导入（假设 common 是顶层模块）
    from common import LayerNorm2d, UpSampleLayer, OpSequential  
except (ImportError, ModuleNotFoundError):
    try:
        # 如果失败则尝试相对导入（假设当前模块在包内）
        from .common import LayerNorm2d, UpSampleLayer, OpSequential  
    except (ImportError, ModuleNotFoundError) as e:
        # 双重导入均失败的兜底方案
        raise ImportError(
            f"无法导入 common 模块（绝对路径和相对路径均失败）\n"
            f"原始错误信息: {str(e)}"
        ) from None'''
import sys
import os
...
from .common import LayerNorm2d, UpSampleLayer, OpSequential
__all__ = ['rep_vit_m1', 'rep_vit_m1_small', 'rep_vit_m2', 'rep_vit_m3', 'RepViT']
...
class RepViTBlock(nn.Module):
    def __init__(self, inp, hidden_dim, oup, kernel_size, stride, use_se, use_hs, skip_downsample=False):
        ...
        if stride == 2 or stride == -1:
            if skip_downsample or stride == -1:
                stride = 1
...
class RepViT(nn.Module):
    ...
    def __init__(self, arch, img_size=1024, fuse=True, freeze=False,
        ...
        if self.fuse:
            ...
            ]) if self.img_size == 1024 else OpSequential([
                nn.Conv2d(stage3_channels, 256, kernel_size=1, bias=False),
            ])
...
def rep_vit_m1_small(img_size=256, **kwargs):
    return RepViT('m1_small', img_size, **kwargs)
